...
public class QRFMessageGenerator extends QRFBase {
	...
	public String createAcceptMessage() {
		return (wrapInXML("<msg><session>" + QRFLinkData.session + "</session><device_identifier>" + MacID
		    + "</device_identifier><acknowledgement_to_server>MESSAGE ACCEPTED</acknowledgement_to_server></msg>"));
	}
	...
	public String createRejectedMessage() {
		return (wrapInXML("<msg><session>" + QRFLinkData.session + "</session><device_identifier>" + MacID
		    + "</device_identifier><acknowledgement_to_server>MESSAGE DROPPED</acknowledgement_to_server></msg>"));
	}
	...
	*/
	...
	public String wrapInXML(String aMessage) {
		...
		//debug("Wrapped string: " + formatted.toString());
	...
	/**
	 * Example from email conversation:
	 * 
	 * <ack part="1" of="34"><name>extra_credit.xml</name></ack>
	 * 
	 * @return
	 */
	public static Element createFileAckMessage(String foundPart,String foundOf,String foundName) {

		QRFBase.staticdebug("createFileAckMessage ()");

		Element root = new Element("ack");

		root.setAttribute("part",foundPart);
		root.setAttribute("of",foundOf);
		root.addContent(createElement("session", QRFLinkData.session));
		
		Element nameElement = new Element("name");
		nameElement.addContent(foundName);
